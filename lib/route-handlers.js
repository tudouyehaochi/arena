...
async function handlePostMessage(req, res, broadcast, runtime) {
  ...
  try {
    ...
    const cachedSeq = await redis.withFallback(async () => {
    ...
    redis.withFallback(() => redis.getClient().set(redisIdemKey, String(msg.seq), 'EX', IDEMPOTENCY_TTL_S), () => {});
...
async function handleGetWsToken(req, res) {
  ...
  const token = auth.issueWsSession(identity, roomId);
