...
function handlePostMessage(req, res, broadcast, runtime) {
  readBody(req, MAX_BODY_BYTES)
    .then(async body => {
      ...
      const redisIdemKey = `arena:idempotency:${dedupeKey}`;
      const cachedSeq = await redis.withFallback(
        async () => {
        ...
        () => {
          ...
          return (hit && hit.expiresAt > Date.now()) ? hit.seq : null;
        },
      );
      if (cachedSeq !== null) {
      ...
      const msg = await store.addMessage({ type: 'chat', from: agentName, content });
      recentCallbacks.set(dedupeKey, { seq: msg.seq, expiresAt: Date.now() + IDEMPOTENCY_TTL_MS });
      redis.withFallback(
        () => redis.getClient().set(redisIdemKey, String(msg.seq), 'EX', IDEMPOTENCY_TTL_S),
        () => {},
      );
      broadcast(msg);
