...
function registerFileTools(server, projectRoot) {
  ...
  server.tool(
    'arena_read_file',
    'Read a file from the project. Default returns full text; use mode="summary" for concise overview, or startLine/endLine for ranges.',
    {
      path: z.string().describe('Relative file path, e.g. "server.js" or "lib/env.js"'),
      mode: z.enum(['full', 'summary']).optional().describe('Read mode: "full" (default) returns entire file, "summary" returns symbols + preview'),
    ...
    async ({ path: filePath, mode, startLine, endLine }) => {
      try {
