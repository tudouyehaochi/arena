...
function createAgentRuntime({ API_URL, INVOCATION_ID, CALLBACK_TOKEN, RUNTIME_ENV, INSTANCE_ID, TARGET_PORT, ROOM_ID, runCommand, MCP_SCRIPT }) {
  ...
  function defaultModelFor(agent) {
    return agent === '明月' ? 'codex' : 'claude';
  }

  function createRoleRuntime(agent) {
    return {
      name: agent,
      ready: true,
      async setup() { console.log(`[mcp] ${agent} inline MCP ready`); },
      async cleanup() { return; },
      canRun() { return this.ready; },
      resolve(prompt, configuredModel) { return resolveRuntime(defaultModelFor(agent), prompt, configuredModel); },
    };
  }

  function ensureRole(agent) {
    const name = String(agent || '').trim();
    if (!name) return null;
    if (!runtimes[name]) runtimes[name] = createRoleRuntime(name);
    return runtimes[name];
  }

  // Backward compatibility
  runtimes.清风 = ensureRole('清风');
  runtimes.明月 = ensureRole('明月');
  runtimes.ensureRole = ensureRole;
  runtimes.listRoleNames = () => Object.keys(runtimes).filter((k) => !['ensureRole', 'listRoleNames'].includes(k));
  return runtimes;
}
