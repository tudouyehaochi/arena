function createAgentRuntime({ API_URL, INVOCATION_ID, CALLBACK_TOKEN, runCommand, MCP_SCRIPT }) {
  const mcpName = `arena_${new URL(API_URL).port || '80'}`;
  ...
  return {
    ...
    明月: {
      ...
      async setup() {
        ...
        try { await runCommand('codex', ['mcp', 'remove', mcpName], '明月 MCP remove(stale)'); } catch {}
        await runCommand('codex', [
          'mcp', 'add', mcpName, '--env', `ARENA_API_URL=${API_URL}`, '--env', `ARENA_INVOCATION_ID=${INVOCATION_ID}`,
      ...
      async cleanup() {
        if (!this.ready) return;
        try { await runCommand('codex', ['mcp', 'remove', mcpName], '明月 MCP remove'); } catch {}
        this.ready = false;
      },
