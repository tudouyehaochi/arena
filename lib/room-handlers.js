...
async function handlePostRooms(req, res, instanceId) {
  ...
  try {
    ...
    const code = err.message === 'invalid_room_id' ? 400 : err.message === 'room_exists' ? 409 : 500;
...
async function handleDeleteRoom(req, res) {
  try {
    ...
    const code = ['cannot_delete_default_room', 'invalid_room_id', 'room_not_found'].includes(err.message) ? 400 : 500;
    jsonResponse(res, code, { error: err.message });
  }
}
