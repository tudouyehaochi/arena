...
function selectSkillCapsules(recentMessages, activeRoleProfile = null) {
  ...
  const roleSkills = Array.isArray(activeRoleProfile?.skills) ? activeRoleProfile.skills : [];
  if (roleSkills.includes('debugging') || roleSkills.includes('incident-response')) selected.push('review');
  if (roleSkills.includes('planning') || roleSkills.includes('code-review')) selected.push('planning');
  if (roleSkills.includes('implementation') || roleSkills.includes('delivery')) selected.push('coding');
...
function buildOptimizedPrompt(agent, recentMessages, options = {}) {
  ...
  const skillSections = skills.map((skillId) => SKILL_CAPSULES[skillId]).filter(Boolean);
  const contextCompact = `## 会话上下文\nsummary=${JSON.stringify(options.sessionSummary || { status: 'empty' })}`;

  const render = (persona, context, activeSkills) => [persona, ...coreSections, ...activeSkills, context].join('\n\n');
  const budget = Number.parseInt(String(options.promptBudgetChars || 0), 10) || 0;
  if (budget <= 0) return render(personaVerbose, sessionContext, skillSections);
