...
function makeStructuredContext(recentMessages, sessionSummary, agentContext, longTermMemory) {
  const normalized = recentMessages.map(normalizeMessage);
  const recent = normalized.slice(-MAX_RECENT);
  const currentGoal = [...normalized].reverse().find((m) => {
  ...
  if (Array.isArray(longTermMemory) && longTermMemory.length > 0) {
    const memoryLine = longTermMemory
      .slice(0, 3)
      .map((m) => `${m.type || 'procedure'}:${clipText(m.summary || '', 80)}`)
      .join(' | ');
    parts.push(`memory=${memoryLine}`);
  }
...
function selectSkillCapsules(recentMessages, activeRoleProfile = null) {
  const selected = [];
  if (isCodingContext(recentMessages)) selected.push({ id: 'coding', priority: 'high', source: 'context' });
  if (isReviewContext(recentMessages)) selected.push({ id: 'review', priority: 'high', source: 'context' });
  if (isPlanningContext(recentMessages)) selected.push({ id: 'planning', priority: 'high', source: 'context' });

  const roleBindings = Array.isArray(activeRoleProfile?.skillBindings)
    ? activeRoleProfile.skillBindings
    : (Array.isArray(activeRoleProfile?.skills) ? activeRoleProfile.skills.map((id) => ({ id, priority: 'medium' })) : []);
  ...
  if (selected.some((s) => s.id === 'debugging' || s.id === 'incident-response')) {
    selected.push({ id: 'review', priority: 'high', source: 'derived' });
  }
  if (selected.some((s) => s.id === 'planning' || s.id === 'code-review')) {
    selected.push({ id: 'planning', priority: 'high', source: 'derived' });
  }
  if (selected.some((s) => s.id === 'implementation' || s.id === 'delivery')) {
    selected.push({ id: 'coding', priority: 'high', source: 'derived' });
  }
  if (selected.some((s) => s.id === 'frontend-design' || s.id === 'ux' || s.id === 'frontend')) {
    selected.push({ id: 'frontend-design', priority: 'high', source: 'derived' });
  }

  const dedup = [];
  for (const skill of selected) {
    if (!SKILL_CAPSULES[skill.id]) continue;
    if (dedup.some((s) => s.id === skill.id)) continue;
    dedup.push(skill);
  }
  const order = { high: 0, medium: 1, low: 2 };
  dedup.sort((a, b) => (order[a.priority] - order[b.priority]) || a.id.localeCompare(b.id));
...
function buildPersonaBlock(agent, activeRoleProfile = null) {
  const rolePersona = String(activeRoleProfile?.persona || '').trim();
  if (rolePersona) return `你是${agent}。\n人设: ${rolePersona}`;
...
function buildLegacyPrompt(agent, recentMessages, options = {}) {
  ...
  const sessionContext = makeStructuredContext(
    recentMessages,
...
function buildOptimizedPrompt(agent, recentMessages, options = {}) {
  ...
  const skills = selectSkillCapsules(recentMessages, options.activeRoleProfile);
  const sessionContext = makeStructuredContext(
    recentMessages,
    options.sessionSummary,
    options.agentContext,
  ...
  const coreSections = [
    ...
    '输出格式: 结论(1句) + 证据(文件/命令) + 下一步(1句)。',
    `发送消息时 from 必须是 "${agent}"。`,
    '请调用 arena_post_message 发送你的回复。',
  ];
  const personaVerbose = buildPersonaBlock(agent, options.activeRoleProfile);
  const personaCompact = `你是${agent}。`;
  const skillSections = skills.map((skill) => ({ ...skill, text: SKILL_CAPSULES[skill.id] })).filter((skill) => !!skill.text);
  const contextCompact = `## 会话上下文\nsummary=${JSON.stringify(options.sessionSummary || { status: 'empty' })}`;

  const render = (persona, context, activeSkills) => [persona, ...coreSections, ...activeSkills.map((s) => s.text), context].join('\n\n');
  const budget = Number.parseInt(String(options.promptBudgetChars || 0), 10) || 0;
  if (budget <= 0) {
    return {
      prompt: render(personaVerbose, sessionContext, skillSections),
      skillUsage: { used: skillSections.map((s) => ({ id: s.id, priority: s.priority })), dropped: [] },
    };
  }

  let activeSkills = skillSections.slice();
  const droppedSkills = [];
  let context = sessionContext;
  let persona = personaVerbose;
  let out = render(persona, context, activeSkills);

  // Trim order: skill blocks -> history/context details -> non-critical persona.
  while (out.length > budget && activeSkills.length > 0) {
    const idx = activeSkills.findLastIndex((s) => s.priority === 'low');
    const fallbackIdx = idx >= 0 ? idx : activeSkills.findLastIndex((s) => s.priority === 'medium');
    const removeIdx = fallbackIdx >= 0 ? fallbackIdx : activeSkills.length - 1;
    const removed = activeSkills.splice(removeIdx, 1)[0];
    if (removed) droppedSkills.push({ id: removed.id, priority: removed.priority, reason: 'budget_trim' });
    out = render(persona, context, activeSkills);
  }
  ...
  return {
    prompt: out,
    skillUsage: {
      used: activeSkills.map((s) => ({ id: s.id, priority: s.priority })),
...
function buildPrompt(agent, recentMessages, options = {}) {
  ...
  return buildOptimizedPrompt(agent, recentMessages, options).prompt;
}

function buildPromptPackage(agent, recentMessages, options = {}) {
  const mode = getPromptMode(options);
  if (mode === PROMPT_MODE_LEGACY) {
    return {
      prompt: buildLegacyPrompt(agent, recentMessages, options),
      skillUsage: { used: [], dropped: [] },
    };
  }
