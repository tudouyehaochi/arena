...
const { listRoomsFromLog, pruneRoomFromLog } = require('./room-log');
...
async function listRooms() {
  const byId = new Map();
  for (const r of listRoomsFromLog(LOG_FILE)) byId.set(r.roomId, r);
  for (const id of stateByRoom.keys()) byId.set(id, { roomId: id, title: id });
