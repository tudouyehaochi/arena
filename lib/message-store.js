...
// Load existing messages from log on startup; seed Redis if needed
async function loadFromLog() {
  // Try loading from Redis first
  const seeded = await redis.withFallback(
    async () => {
      const count = await redisMsg.getMessageCount();
      if (count > 0) {
        const meta = await redisMsg.getMeta();
        const allMsgs = await redisMsg.getRecentMessages(10000);
        for (const msg of allMsgs) {
          messages.push(msg);
        }
        messageSeq = meta.seq;
        consecutiveAgentTurns = meta.agentTurns;
        console.log(`Loaded ${messages.length} messages from Redis (seq=${messageSeq})`);
        return true;
      }
      return false;
    },
  ...
  // Seed Redis from file data
  if (messages.length > 0) {
    await redis.withFallback(
      () => redisMsg.seedFromArray(messages),
      () => {},
    );
  }
}
...
function appendLog(msg) {
  if (!LOG_FILE) return; // disabled in test mode
...
async function addMessage(msg) {
  ...
  // Atomic seq: try Redis INCR, guard against stale Redis seq
  const rawSeq = await redis.withFallback(() => redisMsg.incrSeq(), () => null);
  const redisSeq = rawSeq !== null ? (typeof rawSeq === 'string' ? parseInt(rawSeq, 10) : rawSeq) : null;
  messageSeq++;
  if (redisSeq !== null && redisSeq > messageSeq) {
    messageSeq = redisSeq;
  } else if (redisSeq !== null && redisSeq < messageSeq) {
    redis.withFallback(() => redis.getClient().set('arena:meta:seq', String(messageSeq)), () => {});
  }
  msg.seq = messageSeq;

  // Always dual-write: memory + file
  messages.push(msg);
  appendLog(msg);
...
/** Override log path (for tests). Pass null to disable logging entirely. */
