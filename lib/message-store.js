...
async function listRooms() {
  const localRooms = [...stateByRoom.keys()].map((id) => ({ roomId: id, title: id }));
  return redis.withFallback(
    async () => {
      const rooms = await redisMsg.listRooms();
      if (!rooms.find((r) => r.roomId === DEFAULT_ROOM_ID)) {
        rooms.unshift({ roomId: DEFAULT_ROOM_ID, title: DEFAULT_ROOM_ID, createdAt: null, createdBy: 'system', lastActiveAt: null, boundInstanceId: null });
      }
      return rooms;
    },
    () => localRooms.length > 0 ? localRooms : [{ roomId: DEFAULT_ROOM_ID, title: DEFAULT_ROOM_ID }],
  );
}
