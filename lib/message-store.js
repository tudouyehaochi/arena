...
const { DEFAULT_ROOM_ID, AGENT_NAMES, resolveRoomId } = require('./room');
...
function isAgent(from) {
  ...
  if (agentRegistry.isAgentName(name)) return true;
...
async function ensureDefaultRoomExists() {
  if (await redisMsg.roomExists(DEFAULT_ROOM_ID)) return;
  await redisMsg.ensureRoom(DEFAULT_ROOM_ID, {
    title: DEFAULT_ROOM_ID,
    createdBy: 'system',
    boundInstanceId: process.env.ARENA_INSTANCE_ID || '',
  });
}
...
async function loadFromLog(roomId = DEFAULT_ROOM_ID) {
  ...
  if (!(await redisMsg.roomExists(id))) throw new Error('room_not_found');
  const state = ensureState(id);
  if (state.messages.length > 0) return;
  state.messages = await redisMsg.getRecentMessages(id, 10000);
...
function appendLog(msg) {
  if (!LOG_FILE) return Promise.resolve();
  const { seq, ...logMsg } = msg;
  return new Promise((resolve) => {
    fs.appendFile(LOG_FILE, `${JSON.stringify(logMsg)}\n`, 'utf8', () => resolve());
  });
}
...
async function addMessage(msg, roomId = DEFAULT_ROOM_ID) {
  ...
  const rawSeq = await redisMsg.incrSeq(id);
  const redisSeq = typeof rawSeq === 'string' ? parseInt(rawSeq, 10) : rawSeq;
  state.messageSeq++;
  if (redisSeq > state.messageSeq) state.messageSeq = redisSeq;
  next.seq = state.messageSeq;
  state.messages.push(next);
  if (state.messages.length > MAX_MEMORY_MESSAGES) {
    state.messages = state.messages.slice(-MAX_MEMORY_MESSAGES);
  }
...
async function ensureRoom(roomId, meta = {}) {
  ...
  await ensureDefaultRoomExists();
  ensureState(id);
  await redisMsg.ensureRoom(id, meta);
}
async function roomExists(roomId) {
  const id = resolveRoomId(roomId);
  assertRedisReady();
  await ensureDefaultRoomExists();
  return redisMsg.roomExists(id);
}
async function listRooms() {
  assertRedisReady();
  await ensureDefaultRoomExists();
  const rooms = await redisMsg.listRooms();
  return rooms.sort((a, b) => String(a.roomId).localeCompare(String(b.roomId)));
}
async function deleteRoom(roomId) {
  ...
  await redisMsg.deleteRoom(id);
}
function _setLogFile(p) { LOG_FILE = p; }
async function attachAgentUsage(roomId = DEFAULT_ROOM_ID, agent, usage) {
  ...
  for (let i = state.messages.length - 1; i >= 0; i--) {
    ...
    if (!m.usage) {
      ...
      assertRedisReady();
      await redisMsg.updateMessageUsage(resolveRoomId(roomId), m.seq, m.usage);
...
async function attachAgentSkillUsage(roomId = DEFAULT_ROOM_ID, agent, skillUsage) {
  const state = ensureState(roomId);
  for (let i = state.messages.length - 1; i >= 0; i--) {
    const m = state.messages[i];
    if (m.from !== agent || m.type !== 'chat') continue;
    if (!m.skillUsage) {
      m.skillUsage = {
        used: Array.isArray(skillUsage?.used) ? skillUsage.used.slice(0, 8) : [],
        dropped: Array.isArray(skillUsage?.dropped) ? skillUsage.dropped.slice(0, 8) : [],
      };
      assertRedisReady();
      await redisMsg.updateMessageSkillUsage(resolveRoomId(roomId), m.seq, m.skillUsage);
      return { ok: true, seq: m.seq };
    }
    break;
  }
