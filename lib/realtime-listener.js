...
function getWsToken(apiUrl, authHeader) {
...
function startRealtimeListener({ apiUrl, authHeader, onMessage, onStateChange }) {
  let ws = null;
  let stopped = false;
  ...
  const connect = async () => {
    try {
      const token = await getWsToken(apiUrl, authHeader);
      const wsUrl = `${toWsUrl(apiUrl)}/?token=${encodeURIComponent(token)}`;
