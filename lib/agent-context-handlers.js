...
const { DEFAULT_ROOM_ID, AGENT_NAMES, resolveRoomIdFromUrl, resolveRoomId } = require('./room');

const VALID_AGENTS = AGENT_NAMES;
const FIELD_MAX_LEN = {
  from: 20,
  currentGoal: 500,
  status: 50,
...
async function handleGetAgentContext(req, res) {
  ...
  try {
    ...
    const code = err.message === 'invalid_room_id' ? 400 : err.message === 'redis_unavailable' ? 503 : 500;
    jsonResponse(res, code, { error: err.message });
  }
}
