...
const AGENT_META = {
  清风: { avatar: '清', color: '#2dd4bf' },
  明月: { avatar: '明', color: '#60a5fa' },
};
...
async function handleGetDashboard(req, res) {
  ...
  let seen;
  let route = null;
  let modelMap = agentModelConfig.getDefaultAgentModelMap();
  try {
    ...
    modelMap = await agentModelConfig.getAgentModelMap().catch(() => modelMap);
  } catch (err) {
  ...
  const agents = Object.keys(AGENT_META).map((name) => {
    ...
    return {
      name,
      model: String(modelMap[name] || 'claude').toLowerCase() === 'codex' ? 'Codex' : 'Claude',
      ...AGENT_META[name],
