  ...
  async function ingest(messages, options = {}) {
    ...
    for (const msg of messages || []) {
      ...
      if (mentionTargets.length > 0) {
        ...
        const alwaysOnTargets = roleProfiles
          .filter((role) => String(role.activationMode || 'mention') === 'always_on')
          .sort((a, b) => Number(b.priority || 0) - Number(a.priority || 0))
          .map((role) => role.name);
        if (alwaysOnTargets.length > 0) {
          targets = alwaysOnTargets.map((name) => ({ target: name, reason: 'always_on', ruleId: null }));
        } else {
      ...
      for (const item of targets) {
        ...
        reasonByRole[target] = item.reason;
      }
    }
